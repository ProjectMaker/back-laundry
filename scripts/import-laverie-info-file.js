...
async function readFile() {
  return loadJSON('./import-files/laverie-info/source-1.json')
}
...
async function process() {
  ...
  try {
    while (lines.length) {
      let {address, comments} = lines.pop()
      console.log(address)
  ...
  await fs.writeFileSync('./scripts/import-files/laverie-info/file-1.json', JSON.stringify(newLines, null, 4), 'utf8')
}
